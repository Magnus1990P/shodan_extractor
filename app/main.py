...
@click.option("-o", "--output-dir", multiple=False, help="Output directory", type=click.Path(exists=True, writable=True), default="../data/", required=False)
...
@click.pass_context
def main(ctx, enable_c99:bool=False, enable_whois:bool=False, enable_vt:bool=False, files:List=[], output_dir:str="../data/", ip_list:str="../data/ips.txt"):
...
input_list:   {', '.join(files)}""")
    ...
    if known_ips:
        for shodan_object in shodan_data:
            tag_known_ips(shodan_object, known_ips)
